...
class BaseModel:
    ...
    def setup_schedulers(self):
        ...
        if scheduler_type in ["MultiStepLR", "MultiStepRestartLR"]:
            ...
            for optimizer in self.optimizers:
                self.schedulers.append(
                    lr_scheduler.ReduceLROnPlateauWrapper(
                        optimizer, **train_opt["scheduler"]
                    )
                )
        else:
    ...
    def update_learning_rate(self, current_iter, warmup_iter=-1, metric=None):
        ...
        Args:
            ...
            warmup_iter (int)： Warmup iter numbers. -1 for no warmup.
                Default： -1.
            metric (float, optional): Validation metric for ReduceLROnPlateau.
                Default: None.
        """
        if current_iter > 1:
            for scheduler in self.schedulers:
                # ReduceLROnPlateauWrapper has a step method that accepts metrics
                if isinstance(scheduler, lr_scheduler.ReduceLROnPlateauWrapper):
                    # Only step with metric when provided (at validation time)
                    if metric is not None:
                        scheduler.step(metric)
                    # Otherwise, just increment the internal epoch counter
                    else:
