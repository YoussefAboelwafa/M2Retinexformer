...
class ImageCleanModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get("perceptual_opt"):
            ...
            self.cri_perceptual = cri_percep_cls(**train_opt["perceptual_opt"]).to(
                self.device
            )
        else:
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(enabled=self.use_amp):
            ...
            # additional pixel loss (e.g., MSELoss, PSNRLoss)
            if self.cri_pix2 is not None:
                l_pix2 = 0.0
                for pred in preds:
                    l_pix2 += self.cri_pix2(pred, self.gt)
                loss_dict["l_pix2"] = l_pix2
                l_total = l_total + l_pix2
    ...
    def nondist_validation(
        ...
        return self.metric_results if with_metrics else current_metric
    ...
    def save_best(self, best_metric, param_key="params"):
        ...
        # Build filename with all metrics
        metric_parts = []
        for k, v in best_metric.items():
            if k != "iter":
                metric_parts.append(f"{k}_{v:.2f}")
        metrics_str = "_".join(metric_parts)
        save_filename = f"best_{metrics_str}_iter_{cur_iter}.pth"
