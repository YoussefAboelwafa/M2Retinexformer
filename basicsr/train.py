...
def init_loggers(opt):
    # Use only the experiment name (last part) for log filename, not the full path
    exp_name = osp.basename(opt["name"])
    log_file = osp.join(opt["path"]["log"], f"{exp_name}_{get_time_str()}.log")
    logger = get_root_logger(
        logger_name="basicsr", log_level=logging.INFO, log_file=log_file
    )
    log_file = osp.join(opt["path"]["log"], f"metric.csv")
    logger_metric = get_root_logger(
        logger_name="metric", log_level=logging.INFO, log_file=log_file
    )
    ...
    for phase, dataset_opt in opt["datasets"].items():
        ...
        if phase == "train":
            ...
            train_set = create_dataset(
                dataset_opt
            )  # 将option中的dataset参数传入create_dataset中构建train_set
            # stx()
            train_sampler = EnlargedSampler(
                train_set, opt["world_size"], opt["rank"], dataset_enlarge_ratio
            )  # 缺少关键字 world_size 和 rank，train_sampler是做什么？从get_dist_info得到
            # stx()
            ...
            total_epochs = math.ceil(
                total_iters / (num_iter_per_epoch)
...
def main():
    ...
    # automatic resume ..
    state_folder_path = "experiments/{}/training_states/".format(
        opt["name"]
    )  # 状态路径
    import os
    ...
    if resume_state is None:
        ...
        if (
            ...
            mkdir_and_rename2(osp.join("tb_logger", opt["name"]), opt["rename_flag"])
    ...
    net_g = model.net_g.module if hasattr(model.net_g, "module") else model.net_g
    ...
    logger.info(
        f"Network parameters: {total_params:,} total, {trainable_params:,} trainable"
    )
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if opt.get("val") is not None and (
                ...
                current_metrics = model.validation(
                    val_loader,
                    current_iter,
                    tb_logger,
                    opt["val"]["save_img"],
                ...
                metric_values = ",".join(
                    [str(current_metrics[k]) for k in opt["val"]["metrics"].keys()]
                )
                metric_str = f"{current_iter},{metric_values}"
                logger_metric.info(metric_str)

                # log best metric (using first metric, typically PSNR)
                first_metric_name = list(opt["val"]["metrics"].keys())[0]
                if best_metric[first_metric_name] < current_metrics[first_metric_name]:
                    # Update ALL metrics in best_metric when we achieve a new best
                    for k in opt["val"]["metrics"].keys():
                        best_metric[k] = current_metrics[k]
                    # save best model
                ...
                if tb_logger:
                    ...
                    for k in opt["val"]["metrics"].keys():  # best metrics
                        if k in best_metric:
                            tb_logger.add_scalar(
                                f"metrics/best_{k}", best_metric[k], current_iter
                            )

                # Update ReduceLROnPlateau scheduler with validation metric
                scheduler_type = opt["train"].get("scheduler", {}).get("type", "")
                if scheduler_type == "ReduceLROnPlateau":
                    # Use the metric specified in scheduler config or first validation metric
                    metric_name = opt["train"]["scheduler"].get("metric", first_metric_name)
                    metric_value = current_metrics.get(metric_name, current_metrics[first_metric_name])
                    model.update_learning_rate(current_iter, metric=metric_value)
